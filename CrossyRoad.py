...
native_img = pygame.image.load('images/native-indian_4702021.png').convert_alpha()  # Image 2
native_img = pygame.transform.scale(native_img, (40, 40))

purple_ninja_img = pygame.image.load('images/ninja_4702003.png').convert_alpha()  # Image 1
purple_ninja_img = pygame.transform.scale(purple_ninja_img, (40, 40))
...
while running:
    ...
    if not game_started:
        ...
        char_selection_text = game_font.render("Choose your character:", True, BLACK)
        screen.blit(char_selection_text, (WIDTH // 2 - char_selection_text.get_width() // 2, HEIGHT - 300))

        # Create selection buttons
        button_spacing = 130
        start_x = WIDTH // 2 - (len(characters) * button_spacing) // 2 +50

        for i, character in enumerate(characters):
            button_rect = pygame.Rect(start_x + i * button_spacing, HEIGHT - 250, 50, 50)
            selected = i == selected_character

            # Draw selection highlight
            if selected:
                pygame.draw.rect(screen, (255, 215, 0), button_rect.inflate(10, 10), border_radius=10)

            # Draw character
            screen.blit(character.image, button_rect)
